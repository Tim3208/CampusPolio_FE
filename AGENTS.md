...
- 백엔드 API 명세는 저장소 루트의 `api.csv`에 작성되어 있다.
- API 연동 작업을 할 때는 먼저 `api.csv`에서 Method, URL, 요청 예시, 응답 예시, 성공/실패 코드를 확인한다.
- `api.csv`는 엑셀에서 추출된 CSV이므로 한글이 깨져 보이면 CP949 또는 Windows 기본 인코딩으로 다시 읽는다.
...
- 저장소 전체 규칙은 `AGENTS.md`, `CLAUDE.md`에 반영한다.
